...
// セッションが存在するか確認
async function checkSession(): Promise<ScratchAuthSessionType> {
...
): Promise<ScratchAuthComponentResult<string>> {
  if (!privateCode) {
    return {
      status: false,
...
export const scratchAuthLogin = async (redirect_url?: string) => {
  ...
  redirect(
    `https://auth.itinerary.eu.org/auth/?redirect=${redirectLocation}&name=${config.title}`
  );
};
